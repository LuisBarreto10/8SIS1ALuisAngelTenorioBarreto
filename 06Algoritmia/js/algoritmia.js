...
function problema3() {
  ...
  if (/\s/.test(input)) {
    alert('No se permiten espacios en blanco entre las palabras');
    return;
  }
  ...
  palabras.forEach(function (palabra) {
    var caracteresUnicos = contarCaracteresUnicosEnPalabra(palabra.trim());
    resultado += palabra.trim() + " = " + caracteresUnicos.length + " (" + caracteresUnicos.join("") + ")\n";
  });
